...
void resolve_path_from_var(const std::string& var, std::vector<std::string>& paths)
{
    ...
    if (! wxGetEnv(boost::nowide::widen(var), &wxdirs) || wxdirs.empty() )
        return;
    std::string dirs = boost::nowide::narrow(wxdirs.ToStdWstring());
...
void DesktopIntegrationDialog::perform_desktop_integration()
{
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        ...
        if (i == target_candidates.size() - 1) {
            // create $HOME/.local/share
              create_path(boost::nowide::narrow(wxFileName::GetHomeDir().ToStdWstring()), ".local/share/icons" + icon_theme_dirs);
              // copy icon
    ...
    if(target_dir_icons.empty()) {
    	...
    	// save path to icon
        app_config->set("desktop_integration_icon_slicer_path", GUI::format("%1%/icons/%2%" SLIC3R_APP_KEY "%3%.png", target_dir_icons, icon_theme_path, version_suffix));
    ...
    if (!candidate_found) {
        // create $HOME/.local/share
        create_path(boost::nowide::narrow(wxFileName::GetHomeDir().ToStdWstring()), ".local/share/applications");
        // create desktop file
    ...
    app_config->set("desktop_integration_app_path", GUI::format("%1%/applications/" SLIC3R_APP_KEY "%2%.desktop", target_dir_desktop, version_suffix));
    ...
    if (platform_flavor() != PlatformFlavor::LinuxOnChromium) {
        ...
        if (!target_dir_icons.empty())
        {
            std::string icon_path = GUI::format("%1%/icons/" GCODEVIEWER_APP_KEY "_192px.png",resources_dir());
            std::string dest_path = GUI::format("%1%/icons/%2%" GCODEVIEWER_APP_KEY "%3%.png", target_dir_icons, icon_theme_path, version_suffix);
            if (copy_icon(icon_path, dest_path))
                // save path to icon
        ...
        std::string desktop_file_viewer = GUI::format(
            "[Desktop Entry]\n"
            "Name=" GCODEVIEWER_APP_NAME "%1%\n"
            "GenericName=3D Printing Software\n"
            "Icon=" GCODEVIEWER_APP_KEY "%2%\n"
            ...
            "StartupNotify=false\n", name_suffix, version_suffix, excutable_path);
        std::string desktop_path = GUI::format("%1%/applications/" GCODEVIEWER_APP_KEY "%2%.desktop", target_dir_desktop, version_suffix);
        if (create_desktop_file(desktop_path, desktop_file_viewer))
            ...
            show_error(nullptr, _L("Performing desktop integration failed - could not create Gcodeviewer desktop file. " SLIC3R_APP_KEY " desktop file was probably created successfully."));
        }
    }
...
{
    assert(false);
    return; // slic3r/superslicer: have to test & see exaclty what's doing
