...
void Downloader::on_progress(wxCommandEvent& event)
{
	...
	float percent = (float)std::stoi(boost::nowide::narrow(event.GetString().ToStdWstring())) / 100.f;
...
void Downloader::on_error(wxCommandEvent& event)
{
	...
	NotificationManager* ntf_mngr = wxGetApp().notification_manager();
	ntf_mngr->set_download_URL_error(id, boost::nowide::narrow(event.GetString().ToStdWstring()));
