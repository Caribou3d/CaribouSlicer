    ...
    while (idx_object_layer < object.layers().size() || idx_support_layer < object.support_layers().size()) {
        ...
        if ((layer_to_print.object_layer && layer_to_print.object_layer->has_extrusions())
         ...
         || (layer_to_print.support_layer /* && layer_to_print.support_layer->has_extrusions() */)) {
            ...
            if (object.config().raft_layers.value > 0 && static_cast<int>(layer_to_print.layer()->id()) <= object.config().raft_layers.value) {
                extra_gap = raft_cd;
            }
    ...
    if (custom_gcode != nullptr) {
        ...
        if (color_change) {
            ...
            for (const auto &tuple : status_emitter.stats().color_extruderid_to_used_filament)
                if (tuple.first == static_cast<decltype(tuple.first)>(m600_extruder_before_layer)) 
...
//            if (this->m_layer != nullptr && (this->m_layer->id() < m_config.skirt_height || print.has_infinite_skirt() )) {
            if (this->m_layer != nullptr && (this->m_layer->id() < static_cast<decltype(this->m_layer->id())>(m_config.skirt_height) || print.has_infinite_skirt())) {
...
namespace check_wipe {
    ...
    coord_t max_depth(const ExtrusionPaths& paths, coord_t wipe_inside_depth, coord_t nozzle_width, std::function<Point(coord_t)> compute_point) {
        ...
        if (!success) {
            ...
            for (size_t iter = 0; iter < static_cast<size_t>(nb_iter); ++iter) {
                delta /= 2;
                increased = success;
                if (increased)
                    current_dist += delta;
        ...
        if ((is_hole_loop && this->m_config.seam_notch_inner.get_abs_value(1.) > 0)
            ...
            if (polygon_to_test.size() > 8) {
                ...
                if (is_convex) {
                    ...
                    for (std::vector<Slic3r::Point>::size_type i = 0; i < polygon_to_test.points.size(); ++i) {
...
std::string GCode::extrude_loop(const ExtrusionLoop &original_loop, const std::string &description, double speed)
{
    ...
    for (const ExtrusionPath &path : loop_to_seam.paths)
        // for (int i = 1; i < path.polyline.get_points().size(); ++i)
        for (std::vector<Slic3r::Point>::size_type i = 1; i < path.polyline.get_points().size(); ++i) 
            assert(!path.polyline.get_points()[i - 1].coincides_with_epsilon(path.polyline.get_points()[i]));
    ...
    if (wipe_paths.back().role() == erExternalPerimeter && m_layer != NULL && m_config.perimeters.value > 0 && wipe_paths.front().size() >= 2 && wipe_paths.back().polyline.size() >= 2
        ...
        if (dist_wipe_extra_perimeter > 0) {
            ...
            for (std::vector<Slic3r::ExtrusionPath>::size_type i = 0; i < wipe_paths.size(); ++i) {
        ...
        if (EXTRUDER_CONFIG_WITH_DEFAULT(wipe_inside_end, true)) {
            if (!m_wipe.enable) {
                ...
                for (std::vector<Slic3r::Point>::size_type i = 1; i < original_polygon.points.size(); ++i) 
                ...
                if (!polys.empty()) {
                    ...
                    if (polys.size() > 1) {
                        ...
                        for (std::vector<Slic3r::Polygon>::size_type idx_poly = 0; idx_poly < polys.size(); ++idx_poly) {
                    ...
                    for (std::vector<Slic3r::Point>::size_type idxpt = 1; idxpt < poly.points.size(); ++idxpt) {
                ...
                if (!polys.empty()) {
                    ...
                    for (std::vector<Slic3r::Point>::size_type i = 1; i < poly.points.size(); ++i) 
...
std::string GCode::extrude_path(const ExtrusionPath &path, const std::string &description, double speed_mm_per_sec) {
    ...
    for (std::vector<Slic3r::Point>::size_type i = 1; i < simplifed_path.polyline.get_points().size(); ++i) 
...
double GCode::compute_e_per_mm(double path_mm3_per_mm) {
    ...
    if (str.size() > 2 && !(str.at(0) == '0' && str.at(1) == ' ')) {
        ...
        if (static_cast<std::vector<double>::size_type>(idx_before) >= extrusion_mult.size() - 1) {
