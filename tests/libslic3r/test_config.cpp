...
TEST_CASE("Dynamic config serialization - tests ConfigBase", "[Config]"){
    ...
    ConfigOptionPoints* extruder_offset = config.option<ConfigOptionPoints>("extruder_offset");
    if (extruder_offset != nullptr) {
        std::vector<Vec2d> expected_offsets = {Vec2d{20, 10}};
        CHECK(extruder_offset->m_values == expected_offsets);
    }
...
SCENARIO("Config accessor functions perform as expected.", "[Config]") {
    auto test = [](ConfigBase &config) {
        ...
        WHEN("A numeric option is set to a non-numeric value.") {
            config.set("perimeter_speed", 60.0);
...
SCENARIO("Config ini load/save interface", "[Config]") {
    WHEN("new_from_ini is called") {
        ...
        THEN("Config object contains ini file options.") {
            REQUIRE(config.option_throw<ConfigOptionStrings>("filament_colour", false)->size() == 1);
            // REQUIRE(config.option_throw<ConfigOptionStrings>("filament_colour", false)->front() == "#ABCD");
